...
export default function AdminInventory() {
  ...
  const statCardWidth = width < 420 ? "100%" : "48%";
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const available = data.reduce(
        (acc, curr) => acc + curr.available_copies,
        0,
      );
      return {
        total,
        available,
        outOfStock: data.filter((i) => i.available_copies === 0).length,
      };
    },
  });
  ...
  const runAIAnalysis = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        t("admin.ai_analysis_failed") + ": " + err.message,
      );
    } finally {
    ...
    useMutation({
      ...
      onSuccess: (data: any) => {
        if (data.success) {
          ...
          Alert.alert(
            t("common.error"),
            data.error || t("admin.transfer_failed"),
          );
        }
      },
      onError: (error: any) => {
  ...
  const SuggestionCard = ({ item }: any) => (
    ...
    >
      ...
      >
        ...
        >
          <Text style={{ color: "#4F8EF7", fontSize: 10, fontWeight: "800" }}>
            {t("librarian.ai_suggestion_badge")}
          </Text>
        </View>
        <Text style={{ color: "#5A5F7A", fontSize: 11 }}>
      ...
      {item.metadata?.quantity && (
        ...
        >
          ...
          >
            ...
            >
              {getBranchDisplayName(item.metadata.from_branch)}
            </Text>
          </View>
          ...
          >
            ...
            >
              {getBranchDisplayName(item.metadata.to_branch)}
            </Text>
          </View>
        </View>
      )}
    ...
    <View style={{ flex: 1, backgroundColor: "#0B0F1A" }}>
      ...
      >
        <View>
          ...
          <Text style={{ color: "#8B8FA3", fontSize: 14, marginTop: 4 }}>
            {t("admin.inventory_subtitle")}
          </Text>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          >
            {isAnalyzing ? (
              ...
              <Ionicons name="sparkles" size={24} color="#4F8EF7" />
            )}
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ color: "#8B8FA3", fontSize: 12, fontWeight: "600" }}>
              {t("admin.total_copies")}
            </Text>
            <Text
              style={{
                color: "#FFFFFF",
        ...
        <View style={{ marginBottom: 24 }}>
          ...
          >
            <Text style={{ color: "#FFFFFF", fontSize: 18, fontWeight: "700" }}>
              {t("admin.ai_suggestions")}
            </Text>
            <Text style={{ color: "#4F8EF7", fontSize: 13, fontWeight: "600" }}>
              {t("common.view_all")}
            </Text>
          </View>
          ...
          {branches?.map((branch: any) => (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: "#8B8FA3", fontSize: 12, marginTop: 2 }}>
                  {branch.location}
                </Text>
              </View>
