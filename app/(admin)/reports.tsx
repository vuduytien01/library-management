...
export default function ReportsScreen() {
  ...
  const statBoxWidth = width < 420 ? "100%" : width < 900 ? "48%" : "23.8%";
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        t("messages.fetch_report_failed", "Không thể tải dữ liệu báo cáo: ") +
          error.message,
      );
    } finally {
      setLoading(false);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        t("messages.export_csv_failed", "Không thể xuất file CSV: ") +
          error.message,
      );
    }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        t("messages.export_pdf_failed", "Không thể xuất file PDF: ") +
          error.message,
      );
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            {t(
              "admin.reports_subtitle_month",
              "Phân tích hoạt động thư viện tháng {{month}}",
              { month: selectedMonth },
            )}
          </Text>
        </View>
            ...
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.fetchBtnText}>
                  {t(
                    "admin.generate_report_month",
                    "Tạo báo cáo tháng {{month}}",
                    { month: selectedMonth },
                  )}
                </Text>
              </>
          ...
          <View style={styles.reportContainer}>
            <View style={styles.statsGrid}>
              <View style={[styles.statBox, { width: statBoxWidth }]}>
                <Text style={styles.statLabel}>
                  {t("analytics.total_borrows", "Lượt mượn")}
                </Text>
              ...
              <View style={[styles.statBox, { width: statBoxWidth }]}>
                <Text style={styles.statLabel}>
                  {t("analytics.total_returns", "Lượt trả")}
                </Text>
                <Text style={styles.statValue}>{reportData.returnCount}</Text>
                <Text style={styles.statSubText}>
                  {t("analytics.rate", "Tỉ lệ:")}{" "}
                  {reportData.borrowCount
                    ? Math.round(
                        (reportData.returnCount / reportData.borrowCount) * 100,
                      )
                    : 0}
                  %
                </Text>
              ...
              <View style={[styles.statBox, { width: statBoxWidth }]}>
                <Text style={styles.statLabel}>
                  {t("analytics.fine_revenue", "Tiền phạt")}
                </Text>
                <Text style={[styles.statValue, { color: "#F59E0B" }]}>
                  {reportData.fineRevenue.toLocaleString()}
                </Text>
              ...
              <View style={[styles.statBox, { width: statBoxWidth }]}>
                <Text style={styles.statLabel}>
                  {t("analytics.active_users", "Hoạt động")}
                </Text>
                <Text style={styles.statValue}>{reportData.activeUsers}</Text>
                <Text style={styles.statSubText}>
                  {t("roles.member", "Thành viên")}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.exportSection}>
              <Text style={styles.sectionTitle}>
                {t("admin.export_data", "Xuất dữ liệu")}
              </Text>
