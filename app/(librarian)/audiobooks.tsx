...
export default function LibrarianAudiobooks() {
  ...
  const handleSave = () => {
    const payload: any = {
      ...
      source_platform: formData.source || "fonos",
    ...
    // Auto-inject R2 tag for playback logic
    if (formData.source === "r2") {
      payload.tags = [`r2_path:${formData.source_id}`];
    }

    if (editingAudiobook) {
      audiobooks.update.mutate(
        { id: editingAudiobook.id, ...payload },
        {
          onSuccess: () => {
  ...
  const handleSelectR2File = async (item: any) => {
    setR2Enriching(true);
    try {
      const suggested = await r2.getMetadata(item.key);
      setFormData({
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          {isSuperAdmin && (
            <TouchableOpacity
              ...
              style={[styles.addBtn, { backgroundColor: "#FF9F43" }]}
            >
              ...
              <View style={styles.cardContent}>
                ...
                <View style={styles.info}>
                  ...
                  <Text style={styles.narrator} numberOfLines={1}>
                    {t("audiobooks.voice_prefix")}{" "}
                    {ab.narrator || t("common.unknown")}
                  </Text>
                  <View style={styles.sourceBadge}>
                    <Text style={styles.sourceText}>
                      {(ab.source_platform || ab.source || "N/A").toUpperCase()}
                    </Text>
                  </View>
                </View>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {editingAudiobook
                  ? t("audiobooks.edit_title")
                  : t("audiobooks.add_title")}
              </Text>
              <TouchableOpacity
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.formContainer}>
                <Text style={styles.fieldLabel}>
                  {t("common.book_title").toUpperCase()}
                </Text>
                <TextInput
                ...
                <View style={styles.row}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(val) =>
                        setFormData({ ...formData, source_id: val })
                      }
                ...
                <TouchableOpacity
                  disabled={
                    audiobooks.add.isPending || audiobooks.update.isPending
                  }
                  onPress={handleSave}
              ...
              {r2Prefix !== "" && (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    parts.pop();
                    handleOpenR2(parts.length > 0 ? parts.join("/") + "/" : "");
                  }}
                >
              ...
              {r2Loading ? (
                ...
                <View style={styles.itemsList}>
                  {r2Items.folders.map((f) => (
                    ...
                    >
                      <Ionicons name="folder" size={24} color="#FFD93D" />
                  ...
                  {r2Items.files.map((f) => (
                    <TouchableOpacity
                      key={f.key}
                      disabled={f.isImported || r2Enriching}
                      style={[
                        styles.explorerItem,
                    ...
                    >
                      ...
                      {f.isImported ? (
                        ...
                        <Text style={styles.enrichingLabel}>AI Processing...</Text>
                      ) : (
                        <Ionicons
