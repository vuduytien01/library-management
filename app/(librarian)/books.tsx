...
export default function LibrarianBooks() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const cleanIsbn = booksService.normalizeIsbn(formData.isbn);
      const payload = {
      ...
      const enrichedPayload = await booksService.enrichBookPayload(payload);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (!isMounted) return;
      const syncedBook = (response as any)?.data || response;
      if (syncedBook) {
        const { title, author, cover_url } = syncedBook;
        setFormData({
          ...formData,
          isbn: cleanIsbn,
          ...
          published_date: syncedBook.published_date || formData.published_date,
          category: syncedBook.category || formData.category,
          description: syncedBook.description || formData.description,
          language: syncedBook.language || formData.language,
          average_rating: syncedBook.average_rating
            ? String(syncedBook.average_rating)
            : formData.average_rating,
          edition: syncedBook.edition || formData.edition,
          appendix: syncedBook.appendix || formData.appendix,
          title_en: syncedBook.title_en || formData.title_en,
          title_vi: syncedBook.title_vi || formData.title_vi,
          description_en: syncedBook.description_en || formData.description_en,
          description_vi: syncedBook.description_vi || formData.description_vi,
        });
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            placeholder={t("audiobook.search_placeholder")}
