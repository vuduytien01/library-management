...
export default function AudioPlayerScreen() {
  ...
  const {
    ...
    error,
    refetch,
  } = useAudiobook(id as string);

  useEffect(() => {
    console.log(
      `[AudioPlayer] State: isLoading=${isLoading}, isError=${isError}, hasBook=${!!book}`,
    );
  ...
  const contentWidth =
    Platform.OS === "web" ? Math.min(screenWidth, 420) : screenWidth;
  const diskSize = Math.min(320, Math.max(220, contentWidth * 0.72));
  const playButtonSize = screenWidth < 360 ? 72 : 80;
  const footerHorizontalPadding = screenWidth < 420 ? 24 : 60;
  ...
  useEffect(() => {
    ...
    const sourceUrl = resolveAudiobookCoverUrl(book, "") || null;
    const cacheId = getBookCoverCacheId(book);

    setCoverUrl(sourceUrl || FALLBACK_BOOK_COVER);
    ...
    mediaCache
      ...
      .then((uri) => {
        if (isActive && uri) setCoverUrl(uri);
      })
      .catch((error) => {
        console.warn("[AudioPlayer] Cover cache lookup failed:", error);
      });
    ...
    mediaCache
      ...
      .then((entry) => {
        if (isActive && entry?.uri) setCoverUrl(entry.uri);
      })
      .catch((error) => {
        console.warn("[AudioPlayer] Cover cache write failed:", error);
      });

    return () => {
      isActive = false;
    };
  }, [book?.canonical_cover_url, book?.cover_url, book?.id, book?.title, id]);
      ...
      >
        <TouchableOpacity
          onPress={() =>
            router.canGoBack()
              ? router.back()
              : router.replace("/(member)/audiobooks/index")
          }
      ...
      >
        {errorType === "PERMISSION_DENIED" ? (
          <>
            ...
            >
              {t(
                "audiobook.premium_msg",
                "Nội dung này yêu cầu tài khoản Platinum hoặc quyền truy cập đặc biệt.",
              )}
            </Text>
            <TouchableOpacity
              style={{
            ...
            >
              <Text style={{ color: "#FFFFFF", fontWeight: "700" }}>
                {t("profile.upgrade_now", "Nâng cấp ngay")}
              </Text>
            </TouchableOpacity>
          </>
        ) : isError ? (
          <>
            ...
            >
              {errorType === "QUERY_TIMEOUT"
                ? t(
                    "audiobook.timeout_msg",
                    "Máy chủ phản hồi quá chậm. Vui lòng thử lại sau.",
                  )
                : t(
                    "audiobook.error_loading",
                    "Không thể tải thông tin sách nói. Vui lòng kiểm tra kết nối mạng.",
                  )}
            </Text>
            <TouchableOpacity
              style={{
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            {t(
              "audiobook.missing_data",
              "Xin lỗi, hiện tại không tìm thấy dữ liệu âm thanh cho cuốn sách này.",
            )}
          </Text>
          <TouchableOpacity
            style={{
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={[styles.diskSection, { height: diskSize + 40 }]}>
          <Animated.View
            style={[
              styles.diskWrapper,
              {
          ...
          >
            <Image
              source={{
                uri: coverUrl || FALLBACK_BOOK_COVER,
              }}
              ...
              transition={200}
              onError={() => {
                setCoverUrl(FALLBACK_BOOK_COVER);
              }}
            />
          </Animated.View>
        </View>
        ...
        <View style={styles.infoSection}>
          ...
          {audioError && (
            <View style={styles.errorBanner}>
              ...
              <TouchableOpacity
                onPress={() => load(book, currentChapterIdx || 1)}
              >
...
const styles = StyleSheet.create({
  ...
  footerControls: {
    ...
    paddingTop: 16,
    paddingBottom: Platform.OS === "web" ? 104 : 40,
    backgroundColor: "#0B0F1A",
    zIndex: 100,
  },
  footerBtn: {
