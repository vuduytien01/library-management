...
export default function AudiobookCatalog() {
  ...
  const { data: interactions = [], refetch: refetchInteractions } =
    useInteractions(profile?.id, "AUDIOBOOK");
  const [activeTab, setActiveTab] = useState<"ALL" | "SAVED" | "LIKED">("ALL");
  const [cachedCoverUris, setCachedCoverUris] = useState<
    Record<string, string>
  >({});
  const cardGap = 16;
  const availableGridWidth = Math.max(0, width - 40);
  const audiobookColumns = width < 420 ? 1 : width < 900 ? 2 : 3;
  const audiobookCardWidth =
    (availableGridWidth - cardGap * (audiobookColumns - 1)) / audiobookColumns;
  const audiobookCardHeight = Math.max(
    220,
    Math.min(300, audiobookCardWidth * 1.35),
  );
  ...
  useFocusEffect(
    ...
    }, [profile?.id, refetchInteractions]),
  );
  ...
  const filteredBooks = books?.filter((b: any) => {
    ...
    if (activeTab === "SAVED") {
      return interactions.some(
        (i) =>
          String(i.item_id) === String(b.id) &&
          i.interaction_type === "BOOKMARK",
      );
    }
    if (activeTab === "LIKED") {
      return interactions.some(
        (i) =>
          String(i.item_id) === String(b.id) && i.interaction_type === "LIKE",
      );
    }
    return true;
  });
  ...
  useEffect(() => {
    ...
    const assets =
      books
        ?.map((book: any) => {
          const url = resolveAudiobookCoverUrl(book, "");
          return {
            id: getBookCoverCacheId(book),
            type: "BOOK_COVER" as const,
            title: book.title || String(book.id || "Audiobook"),
            url,
          };
        })
    ...
    mediaCache.prefetchRemoteAssets(assets, 4).then((entries) => {
      if (!isActive) return;
      setCachedCoverUris((current) => {
        const next = { ...current };
        entries.forEach((entry) => {
          next[entry.id] = entry.uri;
        });
        return next;
      });
    });
  ...
  const getLocalizedDuration = (dur?: string) => {
    if (!dur || dur === "0 phút" || dur === "0 minutes")
      return t("audiobook.updating", "Updating...");
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View>
          ...
          <Text style={styles.subtitle}>
            {t("audiobook.subtitle", "Nghe sách mọi lúc, mọi nơi")}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.filterTabs}>
        ...
        >
          ...
          >
            {t("audiobook.filter_saved", "Saved")}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text
            style={[
              styles.filterText,
              activeTab === "LIKED" && styles.activeText,
            ]}
          >
            {t("audiobook.filter_liked", "Liked")}
          </Text>
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.grid}>
            {filteredBooks?.map((book: any, index: number) => (
              <Animated.View
                ...
                style={[styles.cardWrapper, { width: audiobookCardWidth }]}
              >
                <TouchableOpacity
                  style={[styles.card, { height: audiobookCardHeight }]}
                  onPress={() =>
                    router.push(`/(member)/audiobooks/${book.id}` as any)
                  }
                >
                  <Image
                    source={{
                      uri:
                        cachedCoverUris[getBookCoverCacheId(book)] ||
                        resolveAudiobookCoverUrl(book),
                    }}
                    style={styles.cover}
                    contentFit="contain"
                    ...
                    <View style={styles.info}>
                      ...
                      <View style={styles.durationRow}>
                        ...
                        <Text style={styles.durationText}>
                          {getLocalizedDuration(book.duration)}
                        </Text>
                      </View>
                    </View>
                  </LinearGradient>
