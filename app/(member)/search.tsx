...
export default function SearchPage() {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return allBooks.filter((book: any) => {
      ...
      const matchesLanguage =
        ...
        (selectedLangCode && bookLang && String(t(`languages.${bookLang}`)).toLowerCase().includes(selectedLangCode)) ||
        (selectedLanguage && bookLang && String(t(`languages.${bookLang}`)).toLowerCase() === String(t(`languages.${selectedLanguage}`)).toLowerCase());
      ...
      const selectedInEn = String(t(`categories.${selectedCategory}`, { lng: 'en' })).toLowerCase();
      const selectedInVi = String(t(`categories.${selectedCategory}`, { lng: 'vi' })).toLowerCase();
      ...
      const totalAvailable = book.branch_inventory?.reduce(
  ...
  }, [allBooks, debouncedQuery, selectedYear, selectedLanguage, selectedCategory, selectedRating, isAvailableOnly, t]);
    ...
    <View style={styles.container}>
      ...
      >
        <BlurView intensity={80} tint="dark" style={styles.modalOverlay}>
          ...
          />
          <Animated.View entering={FadeInUp} style={styles.aiModalContent}>
            <LinearGradient
                  ...
                  <View style={styles.aiBooksSection}>
                    ...
                    {aiResponse.books.map((book: any) => (
                      ...
                      >
                        <Image
                          source={
                            book.cover_url ? { uri: book.cover_url } : undefined
                          }
