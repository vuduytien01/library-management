  ...
  addToAppleWallet: async (profile: any, cardImageUri: string) => {
    ...
    if (Platform.OS === "web") {
      try {
        ...
        Alert.alert(
          "Thành công",
          "Đã lưu thẻ thành viên vào thiết bị (Apple Pass)!",
        );
        return true;
      } catch (e) {
    ...
    try {
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(cardImageUri, {
          dialogTitle: "Thêm thẻ vào Apple Wallet",
        });
        return true;
      } else {
        Alert.alert(
  ...
  addToGoogleWallet: async (profile: any, cardImageUri: string) => {
    ...
    if (Platform.OS === "web") {
      try {
        ...
        Alert.alert(
          "Thành công",
          "Đã lưu thẻ thành viên vào thiết bị (Google Pass)!",
        );
        return true;
      } catch (e) {
        Alert.alert("Thông báo", "Không thể tải xuống trên trình duyệt");
    ...
    try {
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(cardImageUri, {
          dialogTitle: "Lưu thẻ vào Google Wallet",
        });
        return true;
      } else {
        Alert.alert(
