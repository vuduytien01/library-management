...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  const contentPadding = width < 420 ? 20 : 30;
  const coverSize = Math.min(320, Math.max(220, width * 0.72));
  const controlsWidth = Math.min(width - contentPadding * 2, 420);
  const mainButtonSize = width < 360 ? 72 : 80;
  ...
  const handleSetTimer = () => {
    const options = [
      { label: t("audiobook.turn_off_timer", "Tắt hẹn giờ"), value: null },
      { label: `15 ${t("audiobook.minutes_short", "phút")}`, value: 15 },
      { label: `30 ${t("audiobook.minutes_short", "phút")}`, value: 30 },
      { label: `60 ${t("audiobook.minutes_short", "phút")}`, value: 60 },
      { label: t("audiobook.end_of_chapter", "Hết chương"), value: "end" },
    ];

    Alert.alert(
      t("audiobook.sleep_timer", "Hẹn giờ tắt"),
      t("audiobook.select_timer_desc", "Chọn thời gian tự động dừng phát"),
      options.map((opt) => ({
        text: opt.label,
        onPress: () => {
          if (opt.value === null) {
  ...
  async function setupAudio() {
    try {
      const { Audio, InterruptionModeAndroid, InterruptionModeIOS } =
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.content, { paddingHorizontal: contentPadding }]}>
        ...
        <View style={[styles.sliderContainer, { maxWidth: controlsWidth }]}>
        ...
        <View style={[styles.controls, { maxWidth: controlsWidth }]}>
        ...
        <View style={[styles.bottomActions, { maxWidth: controlsWidth }]}>
          <TouchableOpacity style={styles.actionBtn}>
            <Ionicons name="list" size={20} color="#8A8F9E" />
            <Text style={styles.actionText}>
              {t("audiobook.chapters", "Chương")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={handleSetTimer}>
            ...
            >
              {sleepTimerRemaining
                ...
                : t("audiobook.timer_label", "Hẹn giờ")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
