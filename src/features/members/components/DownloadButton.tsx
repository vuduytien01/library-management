...
export const DownloadButton: React.FC<DownloadButtonProps> = ({
  ...
  const checkStatus = async () => {
    ...
    const file = downloads.find(
      (d: DownloadedFile) => d.id === id && d.type === type,
    );
  ...
  const handleDownload = async () => {
    ...
    try {
      const file = await membersService.downloadFile(
        id,
        title,
        url,
        type,
  ...
  return (
    <TouchableOpacity
      style={[styles.container, downloadedFile && styles.downloadedContainer]}
      ...
      accessibilityHint={
        downloadedFile
          ? "Nhấn để xóa file khỏi thiết bị"
          : "Nhấn để tải sách về đọc offline"
      }
    >
      {isDownloading ? (
