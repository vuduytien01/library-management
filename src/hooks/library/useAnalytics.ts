...
export function useAnalytics() {
  ...
  const peakHoursQuery = useQuery({
    queryKey: ['peak-hours'],
    queryFn: async () => Array.from({ length: 12 }, (_, i) => ({
      hour: (i * 2 + 8) % 24,
  ...
  const deepInsightsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: forecastData } = await supabase.from('book_demand_forecast').select('*').order('confidence', { ascending: false }).limit(10);
  ...
  return useMemo(() => {
    ...
    borrowingHeatmapQuery.data, borrowingHeatmapQuery.status,
    retentionStatsQuery.data, retentionStatsQuery.status,
    inventoryHealthQuery.data, inventoryHealthQuery.status,
    peakHoursQuery.data, peakHoursQuery.status,
    predictedDemandQuery.data, predictedDemandQuery.status,
    deepInsightsQuery.data, deepInsightsQuery.status
  ]);
}
