...
export const localizeItem = (item: any, t: any, language: string) => {
  ...
  const title = isEn ? (item.title_en || item.title) : (item.title_vi || item.title);
  const description = isEn ? (item.description_en || item.description) : (item.description_vi || item.description);
  const author = isEn ? (item.author_en || item.author) : (item.author_vi || item.author);
  const narrator = isEn ? (item.narrator_en || item.narrator) : (item.narrator_vi || item.narrator);
  ...
  if (Array.isArray(categories)) {
    categories = categories.map((c: string) => t(`categories.${c}`, { defaultValue: c }));
  }
  ...
  const interestRecommendationsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const genreMap: Record<string, string[]> = {
        ...
        "Lịch sử": ["History"],
        "Công nghệ": ["Computers", "Technology", "Technology & Engineering", "COMPUTERS", "Application software"],
        "Nghệ thuật": ["Art", "Music", "Performing Arts", "Crafts & Hobbies"],
        "Kinh tế": ["Business & Economics"],
        "Kỹ năng": ["Self-Help", "Psychology", "Health & Fitness", "Family & Relationships"],
        "Truyện tranh": ["Comics & Graphic Novels"],
        "Tiểu thuyết": ["Fiction"],
        "Tâm lý": ["Psychology", "Family & Relationships", "Body, Mind & Spirit"],
        "Triết học": ["Philosophy"],
        "Học ngoại ngữ": ["Foreign Language Study", "Language Arts & Disciplines"],
        "Chính trị - Văn hóa": ["Chính trị - Văn hóa", "Social Science", "Law"],
      };

      const expandedGenres = genres.flatMap(g => genreMap[g] || [g]);
      const list = await booksService.getRecommendationsByGenres(expandedGenres, 10);
      return list.map(localize);
    },
  });
    ...
    queryFn: async () => {
      ...
      const queryPromise = (async () => {
        const { data, error, status } = await supabase
          .from("audiobook_metadata")
          .select("*, book:books(title, author, description, cover_url, category, title_en, title_vi, description_en, description_vi)")
          .eq("id", id)
          .maybeSingle();
        ...
        const enriched = await booksService.enrichWithBookMetadata([data], true);
