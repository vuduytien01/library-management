  ...
  const load = useCallback(
    async (book: any, chapterIdx: number) => {
      ...
      try {
        ...
        const audioCacheId = `${book.id || bookId || "audiobook"}_${chapterIdx}`;
        let playbackUrl = audioUrl;

        try {
          const cachedUri = await mediaCache.getCachedUri(
            audioCacheId,
            "AUDIO",
            audioUrl,
          );
          if (cachedUri) playbackUrl = cachedUri;

          void mediaCache
            .cacheRemoteAsset({
              id: audioCacheId,
              type: "AUDIO",
              title: `${book.title || bookId || "Audiobook"} - ${chapterIdx + 1}`,
              url: audioUrl,
            })
            .catch((error) => {
              console.warn("[Kernel] Audio cache write failed:", error);
            });
        } catch (error) {
          console.warn("[Kernel] Audio cache lookup failed:", error);
        }

        console.log(`[Kernel] Loading: ${playbackUrl}`);
