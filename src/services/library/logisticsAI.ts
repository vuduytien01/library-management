...
export const logisticsAI = {
  ...
  suggestTransfer(
    ...
    const candidates = book.branch_inventory
      .filter((inv) => {
        const branchProvince = PROVINCES_34.find(
          (p) => p.id === inv.branches.province_v2_id,
        );
        return (
          branchProvince &&
          branchProvince.region === targetRegion &&
          inv.available_copies > 1
        );
      })
      .map((inv) => {
        const isIntraProvince =
          inv.branches.province_v2_id === targetProvinceId;
        return {
          fromBranchId: inv.branch_id,
          fromBranchName: inv.branches.name,
          availableCopies: inv.available_copies,
          region: targetRegion,
          distanceTier: (isIntraProvince
            ? "INTRA_PROVINCE"
            : "INTRA_REGION") as "INTRA_PROVINCE" | "INTRA_REGION",
    ...
    if (candidates.length === 0) {
      ...
      return nationalCandidates.sort(
        (a, b) => b.availableCopies - a.availableCopies,
      )[0];
    }
