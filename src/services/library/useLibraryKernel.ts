...
export const useLibraryKernel = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalFine = myBorrows.reduce(
      (acc, b) => acc + (b.estimated_fine || 0),
      0,
    );
  ...
  const canBorrow = (isbn: string) => {
    ...
    if (stats.limitReached)
      return {
        allowed: false,
        reason: `Bạn đã đạt giới hạn mượn (${systemConfig?.max_books} cuốn)`,
      };
    if (stats.hasOverdue)
      return { allowed: false, reason: "Bạn đang có sách quá hạn chưa trả" };

    // Check global inventory (Min 3 copies protection - if required by user)
    const totalCopies =
      book.branch_inventory?.reduce(
        (sum, inv) => sum + (inv.total_copies || 0),
        0,
      ) || 0;
    const availableCopies =
      book.branch_inventory?.reduce(
        (sum, inv) => sum + (inv.available_copies || 0),
        0,
      ) || 0;

    // Rule: Must keep at least 1 copy or follow a minimum copies rule
    if (availableCopies <= 0)
      return { allowed: false, reason: "Sách đã hết bản sao khả dụng" };
