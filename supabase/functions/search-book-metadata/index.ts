...
function upgradeImageUrl(url: string | null): string | null {
  ...
  if (upgraded.includes("books.google.com/books/content")) {
    upgraded = upgraded
      .replace(/zoom=[15]/, "zoom=0")
      .replace(/&edge=curl/, "")
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const rawThumbnail = googleMetadata.imageLinks?.thumbnail || googleMetadata.imageLinks?.smallThumbnail;
