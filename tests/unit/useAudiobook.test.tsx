...
describe("useAudiobook hook (TDD)", () => {
  ...
  it("fails when query takes too long (Timeout)", async () => {
    ...
    (supabase.from as jest.Mock).mockReturnValue({
      ...
      maybeSingle: jest.fn(() => new Promise((resolve) => {
        // Delay resolution by 15s to trigger the 10s timeout
        setTimeout(() => resolve({ data: null, error: null }), 15000);
      }))
    });

    const { result } = renderHook(() => useAudiobook("timeout-id"), { wrapper });

    // Wait up to 12s for the 10s timeout to trigger
    await waitFor(() => {
      if (result.current.error) console.log("Hook Error:", result.current.error.message);
  ...
  it("handles PERMISSION_DENIED (403/RLS)", async () => {
    ...
    const { result } = renderHook(() => useAudiobook("restricted-id"), { wrapper });

    await waitFor(() => {
      if (result.current.error) console.log("Hook Error (Permission):", result.current.error.message);
      expect(result.current.isError).toBe(true);
    }, { timeout: 5000 });
